(function () {
    ...
    (function() {
        function injectPLBadge() {
            try {
                ...
                var isProduct = !!document.querySelector('meta[itemprop="price"]') || !!document.querySelector('h1.nome-produto') || !!document.querySelector('meta[property="og:type"][content*="product"]');
                if (!isProduct) return;
                var b = document.createElement('div');
                b.className = 'pl-seo-badge';
                b.style.cssText = 'text-align:center;padding:4px 0;margin:0;opacity:0.5;line-height:1;';
                var a = document.createElement('a');
                a.href = 'https://provoulevou.com.br?utm_source=widget&utm_medium=lojista&utm_campaign=goog';
                a.target = '_blank';
                a.rel = 'noopener';
    ...
    // ===============================================
    const apiKey = "pl_live_29acb551cde36b0cee64f68fa27ca0b476ba1a275f6c99b5ca1553219eb16368";
    window.PROVOU_LEVOU_API_KEY = apiKey;

    const WEBHOOK_PROVA = 'https://n8n.segredosdodrop.com/webhook/gerador-oculos';
    const WEBHOOK_PIX = 'https://n8n.segredosdodrop.com/webhook/goog-pix';
    const WEBHOOK_PIX_STATUS = 'https://n8n.segredosdodrop.com/webhook/goog-pix-status';
    const WEBHOOK_CHECK_LIMIT = 'https://n8n.segredosdodrop.com/webhook/goog-check-limit';
...
/* PL: borda arredondada do modal */@media(min-width:768px){.q-card-ia,.q-card,#q-card-ia,#q-card,.q-modal-card{border-radius:16px !important;overflow:hidden;}}
            ...
            <div class="q-card-ia">
                ...
                <div class="q-content-scroll">
                    ...
                    <div id="q-header-provador">
                        ...
                        <img src="https://cdn.awsli.com.br/400x300/2630/2630044/logo/logo-preto-8cpw5w49s8.png" alt="Goog" style="height:48px;width:auto;"/>
                    </div>
                    ...
                    <div id="q-loading-box">
                        <div class="q-loading-texts">
                            ...
                            <a href="https://provoulevou.com.br?utm_source=widget&utm_medium=lojista&utm_campaign=goog" target="_blank" class="q-loading-t2">
                ...
                <a href="https://provoulevou.com.br?utm_source=widget&utm_medium=lojista&utm_campaign=goog" target="_blank" class="q-powered-footer">
                    <span>Powered by</span>
    ...
    function buyNow() {
        ...
        try {
            ...
            var _td = (document.querySelector('h1.nome-produto,h1.titulo,.produto-nome h1,h1') || {}).innerText || document.title || '';
        ...
        // Fecha o provador ANTES (senão o mini-carrinho da LI abre ATRÁS do modal)
        try { var _pm = document.getElementById('q-modal-ia'); if (_pm) _pm.style.display = 'none'; document.body.style.overflow = ''; } catch (e) {}
        // Clica o botão nativo da loja (LI faz o add-to-cart via AJAX próprio do tema)
        var sb = findStoreBuyBtn();
        if (sb) { requestAnimationFrame(function () { try { sb.click(); } catch (e) {} }); return; }
        // Fallback: manda pro carrinho
        window.location.href = Q_CHECKOUT_URL;
    }
    ...
    function populateBuyCta() {
        ...
        var prodName = (document.querySelector('h1.nome-produto,h1.titulo,.produto-nome h1,h1') || {}).innerText || document.title || '';
    ...
    function init() {
        ...
        const buyBtn = document.querySelector('.botao-comprar-ajax, .botao-comprar, .comprar button');
        if (buyBtn) {
        ...
        function upgradeImgUrl(url) {
            // Loja Integrada (cdn.awsli.com.br/<WxH>/...): sobe a resolução do path
            if (url.includes('awsli.com.br')) {
                return url.replace(/\/\d{2,4}x\d{2,4}\//, '/1024x1024/');
            }
            return url;
        }
        ...
        function plTrackOpen() { try { fetch(WEBHOOK_OPEN_PL, { method: 'POST', keepalive: true, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ session_id: plSid(), origin: location.origin, produto: (document.querySelector('h1.nome-produto, h1.titulo, .produto-nome h1, h1') || {}).innerText || document.title || '' }) }).catch(function () {}); } catch (e) {} }
            ...
            try {
                ...
                if (pending) {
                    ...
                    const resp = await fetch(WEBHOOK_PIX, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ email: 'cliente@provoulevou.com.br', phone, loja: 'goog', origin: location.origin })
                    });
                    pix = await resp.json();
...
const fd = new FormData();
                    ...
                    console.log('[PL Cicia] Enviando', allProdImgs.length, 'fotos do produto');
                    ...
                    // NENHUMA product_image era enviada -> gerador quebrava ("not a string" no
                    // Extract Product Image) e a prova virava ALTA DEMANDA.
